...
request(Naver_Webtoon_weekday_url, function(err, response, body){
    ...
    for(week_num=0;week_num<7;week_num++){
        ...
        for(webtoon_num=0;webtoon_num<webtoon_total_count;webtoon_num++){
            webtoon_title[webtoon_num]=$('.col').eq(week_num).find('.title').eq(webtoon_num).text().trim();
            webtoon_url[webtoon_num]=Naver_comic_url+$('.col').eq(week_num).find('.title').eq(webtoon_num).attr('href');
            webtoon_img[webtoon_num]=$('.col').eq(week_num).find('img').eq(webtoon_num).attr('src');
        }
