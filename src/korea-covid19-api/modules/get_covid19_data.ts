...
const covid19_api_url = (service_key: string, from: number, to: number, middle_url: string): string =>
...
export const getCovid19Data = async () => {
  ...
  const region_separated_Info = ((): covid19OriginalInfo[][] => {
    const result: covid19OriginalInfo[][] = Array.from(Array(regionCount), () => new Array());
  ...
  const detail_Info = ((): covid19API[] => {
    ...
    for (let regionIndex = 0; regionIndex < regionCount; regionIndex++) {
      ...
      for (let dayIndex = 1; dayIndex < daysCount; dayIndex++) {
        ...
        const new_local_infection_cnt: number = aRegionInfo[dayIndex].new_local_infection; //새로운 지역감염으로 인한 확진자
        const new_overseas_infection_cnt: number = aRegionInfo[dayIndex].new_overseas_infection; //새로운 해외감염으로 인한 확진자
