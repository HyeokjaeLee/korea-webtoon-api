...
export default class get_ai_data {
  static net: any = new brain.recurrent.LSTMTimeStep();
  public new: number[] = [];
  ...
  public date: Date[] = [];
  private get_data = (number_arr: number[]): number[] => {
    let data: number[] = number_arr.slice(number_arr.length - this.input_date_count, number_arr.length);
    let output_data: number[] = [];
    data[data.length - 1] = net.run(data);
    data[data.length - 1] = net.run(data);
    for (let i = 0; i < this.output_data_count; i++) {
      const a_new_data = net.run(data);
  ...
  constructor(data: any[][], model: JSON, input_data_count: number, output_data_count: number) {
    this.input_date_count = input_data_count;
    this.output_data_count = output_data_count;
    net.fromJSON(model);
    data.slice(1).map((_data: any) => {
      this.new.push(_data.confirmed.infected.new.total);
      this.existing.push(_data.confirmed.infected.existing);
      this.total.push(_data.confirmed.infected.existing);
      this.date.push(_data.date);
    });
  ...
  make_test_ai_data() {
    const test_data = this.new.slice(0, this.new.length - this.output_data_count);
    const check_data = this.new.slice(this.new.length - this.output_data_count, this.new.length);
